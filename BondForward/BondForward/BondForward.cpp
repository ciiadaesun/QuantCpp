...
)
{
    ...
    else
    {
        ...
        if (YTMFlag == 0)
        {
            for (i = 0; i < Schedule->NCF; i++)
            {
                ...
                if (TextFlag)
                {
                    DumppingTextData(CalcFunctionName, SaveFileName, "i_cpn", i);
                    DumppingTextData(CalcFunctionName, SaveFileName, "DiscCF_i", DiscCFArray[i]);
                }
            }
        }
        else
    ...
    if (TextFlag) DumppingTextData(CalcFunctionName, SaveFileName, "Bond Price Ended", i);
...
)
{
    ...
    double fbondv = 0.;
    if (TextFlag) DumppingTextData(CalcFunctionName, SaveFileName, "Class Alloc", 1);

    BondPrice = BondLegValue(FXFlag, BondSchd, Rcv_DiscCurve, Rcv_RefCurve, Rcv_FXCurve, ResultRefRate, ResultCPN, ResultDF, ResultDiscCF, 0, nconv, convvolt, convvol, ForwardMaturityDate, fbondv, BondYTMFlag, TextFlag, CalcFunctionName, SaveFileName);
    if (BondYTMFlag == 1)
    {
        n = 0;
    ...
    if (TextFlag) DumppingTextData(CalcFunctionName, SaveFileName, "BondPricing Ended", 1);
...
{
    long i, j;
    char CalcFunctionName[] = "BondForwrad";
    char SaveFileName[100];
    get_filenameYYYYMMDD(SaveFileName, 100, CalcFunctionName);
    if (TextFlag > 0)
    {
        DumppingTextData(CalcFunctionName, SaveFileName, "Nominal", Nominal);
        DumppingTextData(CalcFunctionName, SaveFileName, "Fix0Flo1Flag", Fix0Flo1Flag);
        DumppingTextData(CalcFunctionName, SaveFileName, "PriceDate", PriceDate);
        DumppingTextData(CalcFunctionName, SaveFileName, "RefBondEffectiveDate", RefBondEffectiveDate);
        DumppingTextData(CalcFunctionName, SaveFileName, "RefBondMaturityDate", RefBondMaturityDate);

        DumppingTextData(CalcFunctionName, SaveFileName, "RefBondNumCpnOneYear", RefBondNumCpnOneYear);
        DumppingTextData(CalcFunctionName, SaveFileName, "RefBondCpnRate", RefBondCpnRate);
        DumppingTextData(CalcFunctionName, SaveFileName, "NDayFromBondEndToPay", NDayFromBondEndToPay);
        DumppingTextData(CalcFunctionName, SaveFileName, "ForwardMaturityDate", ForwardMaturityDate);
        DumppingTextData(CalcFunctionName, SaveFileName, "NBondZeroCurveTerm", NBondZeroCurveTerm);

        DumppingTextDataArray(CalcFunctionName, SaveFileName, "BondZeroCurveTerm", NBondZeroCurveTerm, BondZeroCurveTerm);
        DumppingTextDataArray(CalcFunctionName, SaveFileName, "BondZeroCurveRate", NBondZeroCurveTerm, BondZeroCurveRate);
        DumppingTextData(CalcFunctionName, SaveFileName, "BondYTMFlag", BondYTMFlag);
        DumppingTextData(CalcFunctionName, SaveFileName, "BondMarketPriceFlag", BondMarketPriceFlag);
        DumppingTextData(CalcFunctionName, SaveFileName, "BondMarketPrice", BondMarketPrice);

        DumppingTextData(CalcFunctionName, SaveFileName, "ForwardExercisePrice", ForwardExercisePrice);
        DumppingTextData(CalcFunctionName, SaveFileName, "NRiskfreeCurveTerm", NRiskfreeCurveTerm);
        DumppingTextDataArray(CalcFunctionName, SaveFileName, "RiskfreeCurveTerm", NRiskfreeCurveTerm, RiskfreeCurveTerm);
        DumppingTextDataArray(CalcFunctionName, SaveFileName, "RiskfreeCurveRate", NRiskfreeCurveTerm, RiskfreeCurveRate);
        DumppingTextData(CalcFunctionName, SaveFileName, "NFX", NFX);

        DumppingTextDataArray(CalcFunctionName, SaveFileName, "FXTerm", NFX, FXTerm);
        DumppingTextDataArray(CalcFunctionName, SaveFileName, "FXRate", NFX, FXRate);
        DumppingTextData(CalcFunctionName, SaveFileName, "DayCountFlag", DayCountFlag);
        DumppingTextData(CalcFunctionName, SaveFileName, "FixingRateFloatLeg", FixingRateFloatLeg);
        DumppingTextData(CalcFunctionName, SaveFileName, "NHolidays", NHolidays);

        DumppingTextDataArray(CalcFunctionName, SaveFileName, "Holidays", NHolidays, Holidays);
        ...
        DumppingTextData(CalcFunctionName, SaveFileName, "NMaxResultSchedule", NMaxResultSchedule);
    }
    ...
    double* ResultCpnBeforeFMaturity = (double*)malloc(sizeof(double) * (NCPN_BOND + 1));         // 7
    double* ResultDiscCpnBeforeFMaturity = (double*)malloc(sizeof(double) * (NCPN_BOND + 1));     // 8
    
    double* ResultRefRate = (double*)malloc(sizeof(double) * (NCPN_BOND + 1));                    // 9
    double* ResultDiscCF = (double*)malloc(sizeof(double) * (NCPN_BOND + 1));                     // 10
    double* ResultCPN = (double*)malloc(sizeof(double) * (NCPN_BOND + 1));                        // 11
    double* ResultDF = (double*)malloc(sizeof(double) * (NCPN_BOND + 1));                         // 12
    ...
    BNDSchedule->NRefHistory = 1;
    long Temp[1] = { 19000101 };
    double TempR[1] = { 0.03 };
    BNDSchedule->RefHistoryDate = Temp;
    BNDSchedule->RefHistory = TempR;
